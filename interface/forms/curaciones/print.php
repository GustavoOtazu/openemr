...
/**
 * Nursing Wound Care Form - print.php
 * Generates a PDF report using mPDF for a single wound care record.
 *
 * @package   OpenEMR
...
<div class="page">
    ...
    $fields = [
        ...
        xlt('Tracheostomy')        => ['val' => (int)($row['traqueostomia']          ?? 0), 'obs' => $row['obs_traqueostomia']          ?? ''],
        xlt('Ostomies')            => ['val' => (int)($row['ostomias']               ?? 0), 'obs' => $row['obs_ostomias']               ?? ''],
        xlt('Pressure Sores')      => ['val' => (int)($row['escaras']                ?? 0), 'obs' => $row['obs_escaras']                ?? ''],
        xlt('Central Venous Line') => ['val' => (int)($row['via_venosa_central']     ?? 0), 'obs' => $row['obs_via_venosa_central']     ?? ''],
