...
$presion                     = (int)($_POST['presion']                     ?? 0); $obs_presion                  = $_POST['obs_presion']                  ?? '';
$volumen                     = (int)($_POST['volumen']                     ?? 0); $obs_volumen                  = $_POST['obs_volumen']                  ?? '';
$simv                        = (int)($_POST['simv']                        ?? 0); $obs_simv                     = $_POST['obs_simv']                     ?? '';
$psv                         = (int)($_POST['psv']                         ?? 0); $obs_psv                      = $_POST['obs_psv']                      ?? '';
$otros                       = (int)($_POST['otros']                       ?? 0); $obs_otros                    = $_POST['obs_otros']                    ?? '';
$frecuencia_respiratoria     = (int)($_POST['frecuencia_respiratoria']     ?? 0); $obs_frecuencia_respiratoria  = $_POST['obs_frecuencia_respiratoria']  ?? '';
$p_inspiratorio              = (int)($_POST['p_inspiratorio']              ?? 0); $obs_p_inspiratorio           = $_POST['obs_p_inspiratorio']           ?? '';
$p_media                     = (int)($_POST['p_media']                     ?? 0); $obs_p_media                  = $_POST['obs_p_media']                  ?? '';
$p_max                       = (int)($_POST['p_max']                       ?? 0); $obs_p_max                    = $_POST['obs_p_max']                    ?? '';
$chst                        = (int)($_POST['chst']                        ?? 0); $obs_chst                     = $_POST['obs_chst']                     ?? '';
$disparo                     = (int)($_POST['disparo']                     ?? 0); $obs_disparo                  = $_POST['obs_disparo']                  ?? '';
$fvt                         = (int)($_POST['fvt']                         ?? 0); $obs_fvt                      = $_POST['obs_fvt']                      ?? '';
$vol_tidal                   = (int)($_POST['vol_tidal']                   ?? 0); $obs_vol_tidal                = $_POST['obs_vol_tidal']                ?? '';
$vm_programado               = (int)($_POST['vm_programado']               ?? 0); $obs_vm_programado            = $_POST['obs_vm_programado']            ?? '';
$petco2                      = (int)($_POST['petco2']                      ?? 0); $obs_petco2                   = $_POST['obs_petco2']                   ?? '';
$vdvt                        = (int)($_POST['vdvt']                        ?? 0); $obs_vdvt                     = $_POST['obs_vdvt']                     ?? '';
$ko2                         = (int)($_POST['ko2']                         ?? 0); $obs_ko2                      = $_POST['obs_ko2']                      ?? '';
...
if ($is_edit) {
    $check = sqlQuery(
        "SELECT id FROM form_registro_vm WHERE id = ? AND pid = ? AND encounter = ? LIMIT 1",
